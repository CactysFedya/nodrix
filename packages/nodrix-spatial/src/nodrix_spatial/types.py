...
class ImuFrame:
    ...
    def __post_init__(self) -> None:
        if self.timestamp_ns < 0:
            raise ValueError("IMU timestamp cannot be negative")
...
class TransformFrame:
    ...
    def __post_init__(self) -> None:
        if self.timestamp_ns < 0:
            raise ValueError("Transform timestamp cannot be negative")
        if not self.parent_frame_id:
            raise ValueError("parent_frame_id cannot be empty")
        if not self.child_frame_id:
            raise ValueError("child_frame_id cannot be empty")
