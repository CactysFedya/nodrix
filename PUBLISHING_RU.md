...
```bash
git tag -s v1.4.0 -m "Nodrix 1.4.0"
```

Если GPG-подпись не настроена:

```bash
git tag -a v1.4.0 -m "Nodrix 1.4.0"
```
...
Перед запуском добавь секцию `## [1.4.0]` в `CHANGELOG.md`. Скрипт обновит версии, создаст commit/tag и отправит их в GitHub.
