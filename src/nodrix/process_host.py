...
def _decode_ipc_message(
    ...
    *,
    ...
    if not isinstance(value, dict) or not isinstance(value.get("payload"), dict):
        raise ValueError("Invalid Nodrix IPC message envelope")
    try:
        ...
        raise ValueError("Invalid Nodrix IPC message parts") from exc
    if header_size > 1024 or type_size > 4096 or metadata_size > 1024 * 1024:
        raise ValueError("Nodrix IPC header, type, or metadata exceeds its limit")
    ...
    if "shared" in payload_spec:
        ...
        if slot_index is not None:
            ...
            if descriptor.name != slot_descriptor.name:
                raise ValueError("Nodrix output descriptor does not match the reserved slot")
            if descriptor.generation != slot_descriptor.generation:
                raise ValueError("Stale Nodrix output slot generation")
            if not descriptor.readonly:
                raise ValueError("Nodrix process outputs must be read-only")
            relative = descriptor.offset - slot_descriptor.offset
            if relative < 0 or relative + descriptor.size > slot_descriptor.size:
                raise ValueError("Nodrix output descriptor exceeds its reserved slot")
            ...
            if output_slot_buffers is not None and not bool(payload_spec.get("unlink")):
                raise ValueError("Unreserved Nodrix process output must use one-shot shared memory")
        ...
        if payload.nbytes > int(max_payload_bytes):
            raise ValueError(
                f"Nodrix IPC payload exceeds {int(max_payload_bytes)} bytes"
            )
