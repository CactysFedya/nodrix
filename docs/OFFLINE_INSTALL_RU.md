# Офлайн-установка Nodrix 1.0.1

Nodrix 1.0.1 снижает минимальное требование build backend до `setuptools>=68` и не требует `packaging>=24.2` только для чтения лицензии. Это позволяет собирать проект на Ubuntu 24.04/Raspberry Pi без обращения к PyPI, если основные зависимости уже перенесены локально.
